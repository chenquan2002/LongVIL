...
class CodeVerify(Tool):
    ...
    def __init__(self, args):
        ...
        self.base_url = get_api_base_url(args)
        self.object_list=args.object_list
        self.headers = build_headers(self.api_key)
...
Your output must start with one explicit binary consistency result:
- `Verification Result: Yes` if every action-code pair is correct.
- `Verification Result: No` if any action-code pair has an error.
...
- Mapping from natural language to code parameters is fixed: "on top of" -> 'top', "in front of" -> 'front', "to the left of" -> 'left', "to the right of" -> 'right', "behind" -> 'behind', and "into"/"inside" -> 'into'. In particular, 'top' is valid and must not be replaced by 'front' unless the action says "in front of".
...
- If the provided code is already correct, start with `Verification Result: Yes`, do not modify it, and state that it is correct.
- If any provided code is incorrect, start with `Verification Result: No`, explain the mismatch, and provide corrected code.
...
"""
    ...
    def forward(self, action_list, python_code):
        ...
        log_token_usage(f"tool-{self.name}", response_data)
